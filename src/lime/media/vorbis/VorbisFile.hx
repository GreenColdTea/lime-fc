...
class VorbisFile
{
	...
	// The native decoder reads directly out of this buffer for as long as the file is open
	// (it isn't copied), so a strong reference must be kept here to stop the GC from collecting
	// it out from under an in-progress read.
	@:noCompletion private var __srcBytes:Bytes;
